  ...
  def getTheveninEquiv(self,excluded=[]):
    ...
    for connection in self.connections:
      ...
      if not connection in excluded:
        v,r=connection.getTheveninEquiv(excluded+[self])#If we've not been told to exclude it follow the link to a simplified source.
        ...
        connectedDevices.add( (v,r,i) ) #Add the result (and the counter) to the set of connections
    ...
    for device in connectedDevices: #For everything we're connected to
      ...
      for v,r,_ in connectedDevices.difference( set( [device] ) ): #Consider everything else as a voltage and resistance
        resistanceProduct*=r 
